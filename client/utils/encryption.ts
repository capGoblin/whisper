 ...
 */
export const computeSharedSecret = (privateKey: string, publicKey: string): string => {
  try {
    ...
    const privateKeyBytes = new Uint8Array(cleanPrivateKey.match(/.{2}/g)!.map(byte => parseInt(byte, 16)));
    
    // Create public key point
    const publicKeyPoint = secp.Point.fromHex(cleanPublicKey);
    
    // Compute shared secret using ECDH
    const sharedSecretPoint = secp.getSharedSecret(privateKeyBytes, publicKeyPoint.toBytes(), true);
...
export const extractViewTag = (hashedSharedSecret: string): string => {
  // Remove 0x prefix if present
  const cleanSecret = hashedSharedSecret.startsWith('0x') ? hashedSharedSecret.slice(2) : hashedSharedSecret;
 ...
 */
export const encryptMessage = async (message: string, sharedSecret: string): Promise<string> => {
  try {
    // Convert shared secret to Uint8Array
    const cleanSecret = sharedSecret.startsWith('0x') ? sharedSecret.slice(2) : sharedSecret;
    const secretBytes = new Uint8Array(cleanSecret.match(/.{2}/g)!.map(byte => parseInt(byte, 16)));
 ...
 */
export const decryptMessage = async (encryptedHex: string, sharedSecret: string): Promise<string | null> => {
  try {
    // Convert shared secret to Uint8Array
    const cleanSecret = sharedSecret.startsWith('0x') ? sharedSecret.slice(2) : sharedSecret;
    const secretBytes = new Uint8Array(cleanSecret.match(/.{2}/g)!.map(byte => parseInt(byte, 16)));
    ...
    const cleanEncrypted = encryptedHex.startsWith('0x') ? encryptedHex.slice(2) : encryptedHex;
    const encryptedBytes = new Uint8Array(cleanEncrypted.match(/.{2}/g)!.map(byte => parseInt(byte, 16)));
 ...
 */
export const prepareMetadata = async (message: string, sharedSecret: string): Promise<string> => {
  try {
    // Extract view tag
    ...
    const encryptedHex = encryptedMessage.startsWith('0x') ? encryptedMessage.slice(2) : encryptedMessage;
 ...
 */
export const parseMetadata = (metadata: string): { viewTag: string; encryptedMessage: string } => {
  try {
 ...
 * @returns Decrypted message or null if decryption fails
 */
export const decryptMetadata = async (metadata: string, sharedSecret: string): Promise<string | null> => {
  try {
    const { encryptedMessage } = parseMetadata(metadata);
    return await decryptMessage(encryptedMessage, sharedSecret);
  } catch (error) {
 ...
 */
export const checkViewTagMatch = (metadata: string, expectedViewTag: string): boolean => {
  try {
 ...
 */
export const generateEncryptionKey = async (sharedSecret: string): Promise<CryptoKey> => {
  try {
    const cleanSecret = sharedSecret.startsWith('0x') ? sharedSecret.slice(2) : sharedSecret;
    const secretBytes = new Uint8Array(cleanSecret.match(/.{2}/g)!.map(byte => parseInt(byte, 16)));
