 ...
 */
export const validateMessageInput = (message: string): {
  valid: boolean;
  errors: string[];
  warnings: string[];
} => {
  ...
  if (!lengthCheck.valid) {
    errors.push(`Message too long. Maximum ${lengthCheck.maxLength} characters allowed.`);
 ...
 */
export const validateRecipientInput = (input: string): {
  type: 'meta-address' | 'eth-address' | 'ens' | 'invalid';
  valid: boolean;
  error?: string;
} => {
  if (!input || typeof input !== 'string') {
    return { type: 'invalid', valid: false, error: 'Input is required' };
  }
  ...
  return { 
    type: 'invalid', 
    valid: false, 
    error: 'Invalid format. Enter a stealth meta-address (st:eth:0x...), Ethereum address (0x...), or ENS name (...eth)' 
  };
};
